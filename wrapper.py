...
class Wrapper:
    ...
    def __init__(self, args):
        ...
        """
        self.args = args
        if self.args.feature_backbone in ['resnet101', 'resnet152', 'resnet50', 'resnet50_moco', 'adv_inception_v3', 'inception_v3', 'virtex', 'virtex2', 'dino_resnet50']:
            image_feature_size = 2048
        elif self.args.feature_backbone in ['googlenet', 'shufflenet', 'vit_large', 'resnet50_clip', 'resnet50x64_clip', 'mlp_mixer_l16', 'vit_large_21k']:
            image_feature_size = 1024
        elif self.args.feature_backbone in ['vgg16', 'alexnet']:
            image_feature_size = 4096
        elif self.args.feature_backbone in ['CLIP', 'resnet101_clip', 'vit_b32_clip', 'vit_b16_clip', 'vq_vae_fromScratch']: ## This is CLIP
        ...
        'dino_vitb16', 'soho', 'vit_l14_clip_finetune_v2', 'vit_l14_clip_finetune_classAndAtt', 'vit_l14_clip_finetune_class200Epochs', 
        ...
        elif self.args.feature_backbone in ['combinedv1']:
            image_feature_size = 768 + 768
        elif self.args.feature_backbone in ['combinedv2']:
            image_feature_size = 2048 + 768
        # # elif self.args.feature_backbone in ['vq_vae_fromScratch']:
        # #     image_feature_size = 32768
        # elif self.args.feature_backbone in ['clip_l14_ImageAndAtts']:
        #     image_feature_size = 312
        # elif self.args.feature_backbone in ['clip_l14_ImageAndAtts_v2']:
        #     image_feature_size = 768 + 312
        ...
        if self.args.methods == 'UPPER_BOUND':
            ...
            opt = { 'dataset': 'CUB', 
                    'dataroot': '/home/pc51/ZERO_SHOT/CADA-VAE-PyTorch/data/SDGZSL_data', 
                    'workers': 4, 'image_embedding': 'res101', 'class_embedding': 'att', 
                    'gen_nepoch': 600, 'lr': 3e-05, 'zsl': False, 'finetune': True, 'ga': 1.0, 
                    'beta': 0.003, 'weight_decay': 1e-08, 'dis': 0.3, 'dis_step': 3.0, 'kl_warmup': 0.001, 
                    'tc_warmup': 0.0001, 'vae_dec_drop': 0.1, 'vae_enc_drop': 0.4, 'ae_drop': 0.0, 
                    'classifier_lr': 0.002, 'classifier_steps': 50, 'batchsize': 64, 'nSample': 1200, 
                    'disp_interval': 200, 'save_interval': 10000, 'evl_interval': 65990, 'evl_start': 0, 
                    'manualSeed': 5606, 'latent_dim': 20, 'q_z_nn_output_dim': 128, 'S_dim': image_feature_size, 'NS_dim': image_feature_size, 
                    'gpu': 'cuda', 'C_dim': 312, 'X_dim': image_feature_size, 'Z_dim': 20, 'y_dim': 150}
            self.model = SDGZSL_VAE(opt)
            self.relationNet = SDGZSL_RelationNet(opt)
            self.discriminator = SDGZSL_Discriminator(opt)
            self.ae = SDGZSL_AE(opt)

            self.optimizer = torch.optim.Adam(self.model.parameters(), lr=opt.lr, weight_decay=opt.weight_decay)
            self.relation_optimizer = torch.optim.Adam(self.relationNet.parameters(), lr=opt.lr, weight_decay=opt.weight_decay)
            self.dis_optimizer = torch.optim.Adam(self.discriminator.parameters(), lr=opt.lr, weight_decay=opt.weight_decay)
            self.ae_optimizer = torch.optim.Adam(self.ae.parameters(), lr=opt.lr, weight_decay=opt.weight_decay)
